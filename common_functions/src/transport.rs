...
pub const TRANSPORT_FONTS: [[f64; 4]; 8] = [
	[   8.0,   4.0, 100.00,   6.0 ],	// FONT_NORMAL:			[ FA_SIZE, FA_SUP_SIZE, FA_Y_PERCENTAGE, FA_HEIGHT ]
	[   8.0,   4.0,  75.00,   8.0 ],	// FONT_UNDERLINED: 	[ FA_SIZE, FA_SUP_SIZE, FA_Y_PERCENTAGE, FA_HEIGHT ]
	[   8.0,   4.0, 100.00,   8.0 ],	// FONT_OVERLINED:		[ FA_SIZE, FA_SUP_SIZE, FA_Y_PERCENTAGE, FA_HEIGHT ]
...
pub fn generate_pp_number_element(mut counter_file: &std::fs::File, pp: &TextField, x_position: f64, y_position: f64, anchor: &String) -> f64
{
	...
	if !pp.text.is_empty() {
		...
		write!(counter_file, "\t\t<text x=\"0.00\" y=\"{0}%\" dominant-baseline=\"auto\" text-anchor=\"{1}\"><tspan style=\"font-size:{2:.2}px;{FONT_WEIGHT_BOLD};font-family:{3};fill:{4};fill-opacity:1;stroke-width:0.2\">{5}{6}{7}</tspan></text>\n", pp.fonts.y_percentage(), anchor, pp.fonts.size(), FONT_MAIN, pp.color, prefix_note, pp.text, postfix_note).unwrap();
